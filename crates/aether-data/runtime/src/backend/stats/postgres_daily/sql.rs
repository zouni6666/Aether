...
SELECT
    CAST(COUNT(id) AS BIGINT) AS cache_hit_total_requests,
    CAST(
        COUNT(id) FILTER (
            WHERE GREATEST(COALESCE(cache_read_input_tokens, 0), 0) > 0
        ) AS BIGINT
    ) AS cache_hit_requests,
    CAST(COUNT(id) FILTER (WHERE status = 'completed') AS BIGINT)
        AS completed_total_requests,
    CAST(
        COUNT(id) FILTER (
            WHERE status = 'completed'
              AND GREATEST(COALESCE(cache_read_input_tokens, 0), 0) > 0
        ) AS BIGINT
    ) AS completed_cache_hit_requests,
    CAST(
        COALESCE(
            SUM(GREATEST(COALESCE(input_tokens, 0), 0)) FILTER (
                WHERE status = 'completed'
            ),
            0
        ) AS BIGINT
    ...
    CAST(
        COALESCE(
            SUM(GREATEST(COALESCE(cache_read_input_tokens, 0), 0)) FILTER (
                WHERE status = 'completed'
            ),
            0
        ) AS BIGINT
    ...
    CAST(
        COALESCE(
            SUM(COALESCE(CAST(cache_creation_cost_usd AS DOUBLE PRECISION), 0)) FILTER (
                WHERE status = 'completed'
            ),
            0
        ) AS DOUBLE PRECISION
    ...
    CAST(
        COALESCE(
            SUM(COALESCE(CAST(cache_read_cost_usd AS DOUBLE PRECISION), 0)) FILTER (
                WHERE status = 'completed'
            ),
            0
        ) AS DOUBLE PRECISION
    ...
    CAST(
        COALESCE(
            SUM(COALESCE(CAST(total_cost_usd AS DOUBLE PRECISION), 0)) FILTER (
                WHERE billing_status = 'settled' AND COALESCE(CAST(total_cost_usd AS DOUBLE PRECISION), 0) > 0
            ),
            0
        ) AS DOUBLE PRECISION
    ) AS settled_total_cost,
    CAST(COUNT(id) FILTER (WHERE billing_status = 'settled' AND COALESCE(CAST(total_cost_usd AS DOUBLE PRECISION), 0) > 0) AS BIGINT)
    ...
    CAST(
        COALESCE(
            SUM(GREATEST(COALESCE(output_tokens, 0), 0)) FILTER (
                WHERE billing_status = 'settled' AND COALESCE(CAST(total_cost_usd AS DOUBLE PRECISION), 0) > 0
            ),
            0
        ) AS BIGINT
    ...
    CAST(
        COALESCE(
            SUM(GREATEST(COALESCE(cache_creation_input_tokens, 0), 0)) FILTER (
                WHERE billing_status = 'settled' AND COALESCE(CAST(total_cost_usd AS DOUBLE PRECISION), 0) > 0
            ),
            0
        ) AS BIGINT
    ...
    MIN(CAST(EXTRACT(EPOCH FROM finalized_at) AS BIGINT)) FILTER (
        WHERE billing_status = 'settled' AND COALESCE(CAST(total_cost_usd AS DOUBLE PRECISION), 0) > 0
    ) AS settled_first_finalized_at_unix_secs,
    MAX(CAST(EXTRACT(EPOCH FROM finalized_at) AS BIGINT)) FILTER (
        WHERE billing_status = 'settled' AND COALESCE(CAST(total_cost_usd AS DOUBLE PRECISION), 0) > 0
    ...
    CAST(
        COALESCE(SUM(output_tokens) FILTER (WHERE status NOT IN ('pending', 'streaming') AND provider_name NOT IN ('unknown', 'pending')), 0) AS BIGINT
    ) AS output_tokens,
    CAST(
        COALESCE(
    ...
    CAST(
        COALESCE(
            SUM(cache_creation_input_tokens_5m) FILTER (WHERE status NOT IN ('pending', 'streaming') AND provider_name NOT IN ('unknown', 'pending')),
            0
        ) AS BIGINT
    )
    ...
    CAST(
        COALESCE(
            SUM(cache_creation_input_tokens_1h) FILTER (WHERE status NOT IN ('pending', 'streaming') AND provider_name NOT IN ('unknown', 'pending')),
            0
        ) AS BIGINT
    )
    ...
    CAST(
        COALESCE(
            SUM(cache_read_input_tokens) FILTER (WHERE status NOT IN ('pending', 'streaming') AND provider_name NOT IN ('unknown', 'pending')),
            0
        ) AS BIGINT
    ...
    CAST(
        COALESCE(SUM(total_cost_usd) FILTER (WHERE status NOT IN ('pending', 'streaming') AND provider_name NOT IN ('unknown', 'pending')), 0)
            AS DOUBLE PRECISION
    ) AS total_cost,
    CAST(
        COALESCE(SUM(actual_total_cost_usd) FILTER (WHERE status NOT IN ('pending', 'streaming') AND provider_name NOT IN ('unknown', 'pending')), 0)
            AS DOUBLE PRECISION
    ) AS actual_total_cost,
    CAST(
        COALESCE(SUM(input_cost_usd) FILTER (WHERE status NOT IN ('pending', 'streaming') AND provider_name NOT IN ('unknown', 'pending')), 0)
            AS DOUBLE PRECISION
    ) AS input_cost,
    CAST(
        COALESCE(SUM(output_cost_usd) FILTER (WHERE status NOT IN ('pending', 'streaming') AND provider_name NOT IN ('unknown', 'pending')), 0)
            AS DOUBLE PRECISION
    ) AS output_cost,
    CAST(
        COALESCE(SUM(cache_creation_cost_usd) FILTER (WHERE status NOT IN ('pending', 'streaming') AND provider_name NOT IN ('unknown', 'pending')), 0)
            AS DOUBLE PRECISION
    ) AS cache_creation_cost,
    CAST(
        COALESCE(SUM(cache_read_cost_usd) FILTER (WHERE status NOT IN ('pending', 'streaming') AND provider_name NOT IN ('unknown', 'pending')), 0)
            AS DOUBLE PRECISION
    ) AS cache_read_cost,
    COALESCE(
        SUM(
    ...
    CAST(
        COALESCE(AVG(response_time_ms) FILTER (WHERE status NOT IN ('pending', 'streaming') AND provider_name NOT IN ('unknown', 'pending')), 0)
            AS DOUBLE PRECISION
    ) AS avg_response_time_ms,
    CAST(
        COUNT(DISTINCT model) FILTER (WHERE status NOT IN ('pending', 'streaming') AND provider_name NOT IN ('unknown', 'pending')) AS BIGINT
    ) AS unique_models,
    CAST(
        COUNT(DISTINCT provider_name) FILTER (WHERE status NOT IN ('pending', 'streaming') AND provider_name NOT IN ('unknown', 'pending')) AS BIGINT
    ) AS unique_providers
...
mod tests {
    ...
    fn daily_aggregate_uses_one_time_bounded_base_table_scan() {
        let sql = SELECT_STATS_DAILY_AGGREGATE_SQL;

        assert_eq!(sql.matches("FROM usage_billing_facts AS usage").count(), 1);
        assert_eq!(sql.matches("WHERE created_at >= $1").count(), 1);
        assert_eq!(sql.matches("AND created_at < $2").count(), 1);
        assert!(!sql.contains("MATERIALIZED"));
        assert!(!sql.contains("bucket_usage"));
    }

    #[test]
    fn daily_aggregate_keeps_independent_row_populations() {
        let sql = SELECT_STATS_DAILY_AGGREGATE_SQL;

        assert_eq!(sql.matches("status = 'completed'").count(), 8);
        assert_eq!(sql.matches("billing_status = 'settled'").count(), 8);
        assert_eq!(
            sql.matches("COALESCE(CAST(total_cost_usd AS DOUBLE PRECISION), 0) > 0")
                .count(),
            8
        );
        assert_eq!(
    ...
    #[test]
    fn daily_aggregate_preserves_the_decoder_column_contract() {
        let sql = SELECT_STATS_DAILY_AGGREGATE_SQL;
        let aliases = [
            "cache_hit_total_requests",
            "cache_hit_requests",
