...
fn print_usage() {
    eprintln!(
        "usage: cargo run -p aether-integration-tests --bin mock_openai_upstream -- [--bind 127.0.0.1:18181]... [--chunks 8] [--first-byte-delay-ms 0] [--chunk-delay-ms 20] [--payload-bytes 32] [--status 200] [--assume-stream]"
    );
}
